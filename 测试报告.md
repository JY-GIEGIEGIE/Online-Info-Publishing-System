...
> 后端全部 37 个单元测试汇总结果。因 Maven 终端输出包含 MyBatis SQL 日志、Spring 启动信息等大量滚动内容不便直接截图，故通过 `mvn test` 捕获 Surefire 统计数据，生成 `screenshots/unit_test_result.html` 可视化页面后截图。具体数据文件见 `screenshots/test_run_raw.txt`。
...
![API响应采集](screenshots/test_results.html)
> 边界值测试通过 curl 调用各边界接口，API 原始 JSON 响应保存在 `screenshots/` 目录下（`search_maotai.json`、`quote_standard.json`、`user_standard.json` 等），可用浏览器打开 `screenshots/test_results.html` 查看汇总。
...
联调监控详细数据见项目根目录 `INTEGRATION_TEST_LOG.md`（10 分钟/20 轮/6 只股票完整记录）。前端各角色视图截图如下：
